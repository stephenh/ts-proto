 ...
 */
export function generateNtrvlService(
  ctx: Context,
  ...
  serviceDesc.method.forEach((methodDesc, index) => {
    ...
    const inputType = requestType(ctx, methodDesc, false);
    params.push(code`request: ${inputType}`);

    // Use metadata as last argument for interface only configuration
    params.push(code`headers?: any`);
...
function generateRegularRpcMethod(ctx: Context, methodDesc: MethodDescriptorProto): Code {
  ...
  const returnStatement = createNtrvlServiceReturn(ctx, methodDesc, decode, errorHandler);
...
function createNtrvlServiceReturn(
  ctx: Context,
...
export function generateNtrvlServiceClientImpl(
  ctx: Context,
  fileDesc: FileDescriptorProto,
  serviceDesc: ServiceDescriptorProto,
): Code {
 ...
 * types.
 */
export function generateNtrvlRpcType(ctx: Context, hasStreamingMethods: boolean): Code {
  const { options } = ctx;
