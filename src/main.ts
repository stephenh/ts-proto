...
export function generateFile(typeMap: TypeMap, fileDesc: FileDescriptorProto, parameter: string): FileSpec {
  ...
  file = addConfigureProtobuf(file, options);
...
function addConfigureProtobuf(file: FileSpec, options: Options): FileSpec {
  if (!options.configureProtobuf) {
    return file;
  }
  return file.addCode(
      CodeBlock.empty()
