...
export function defaultValue(typeMap: TypeMap, field: FieldDescriptorProto, options: Options): any {
  if (options.useOptionals && isOptional(field)) {
    return 'undefined';
  }
  switch (field.type) {
    case FieldDescriptorProto.Type.TYPE_DOUBLE:
...
export function messageToTypeName(typeMap: TypeMap, protoType: string, options: (Options | null) = null, keepValueType: boolean = false): TypeName {
  ...
  if (!keepValueType && protoType in valueTypes) {
    ...
    if (!options || !options.useOptionals) {
      // When useOptionals=false (the default), we union the type with
      // undefined to indicate the value is optional. If useOptionals=true,
      // all non-scalar types are already optional properties, so there's no
      // need for it here.
...
export function toTypeName(typeMap: TypeMap, messageDesc: DescriptorProto, field: FieldDescriptorProto, options: Options): TypeName {
  ...
  if (isRepeated(field)) {
  ...
  } else if (isOptional(field) && !isValueType(field) && !options.useOptionals) {
    ...
    // clause are already optional, so no need for the type union here.
