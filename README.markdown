...
* With `--ts_proto_opt=useOptionals=true`, non-scalar fields and fields that are part of a `oneof` clause are declared as optional TypeScript properties, e.g. `field?: Message` instead of `field: Message | undefined`.
