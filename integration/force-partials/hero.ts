...
export interface HeroService {
  FindOneHero(request: DeepPartial<HeroById>): Promise<Hero>;
  FindOneVillain(request: DeepPartial<VillainById>): Promise<Villain>;
  FindManyVillain(request: Observable<DeepPartial<VillainById>>): Observable<Villain>;
}
...
export class HeroServiceClientImpl implements HeroService {
  ...
  FindOneHero(request: DeepPartial<HeroById>): Promise<Hero> {
    const data = HeroById.encode(HeroById.fromPartial(request)).finish();
    const promise = this.rpc.request("hero.HeroService", "FindOneHero", data);
    return promise.then((data) => Hero.decode(new _m0.Reader(data)));
  }

  FindOneVillain(request: DeepPartial<VillainById>): Promise<Villain> {
    const data = VillainById.encode(VillainById.fromPartial(request)).finish();
  ...
  FindManyVillain(request: Observable<DeepPartial<VillainById>>): Observable<Villain> {
    const data = request.pipe(map((request) => VillainById.encode(VillainById.fromPartial(request)).finish()));
