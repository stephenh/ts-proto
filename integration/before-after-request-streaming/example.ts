...
export const DashStateServiceName = "rpx.DashState";export class DashStateClientImpl implements DashState {private readonly rpc: Rpc;private readonly service: string;constructor(rpc: Rpc, opts?: {service?: string}) {this.service = opts?.service || DashStateServiceName;this.rpc = rpc;this.UserSettings = this.UserSettings.bind(this);this.ActiveUserSettingsStream = this.ActiveUserSettingsStream.bind(this);this.ChangeUserSettingsStream = this.ChangeUserSettingsStream.bind(this);}
    ...
    if (this.rpc.beforeRequest) {
      ...
      return result.pipe(map((data) => { try {
    ...
    ChangeUserSettingsStream(
      request: Observable<DashUserSettingsState>
    ): Observable<DashUserSettingsState> {
      const data = request.pipe(map(request => {const encodedRequest = DashUserSettingsState.encode(request).finish(); 
    if (this.rpc.beforeRequest) {
