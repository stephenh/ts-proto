...
const baseCred: object = { id: '' };

export const Cred = {
  encode(message: Cred, writer: Writer = Writer.create()): Writer {
    if (message.id !== '') {
  ...
  decode(input: Reader | Uint8Array, length?: number): Cred {
    const reader = input instanceof Reader ? input : new Reader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const DashStateChangeUserSettingsStreamDesc: UnaryMethodDefinitionish = {
  ...
  requestType: {
    serializeBinary() {
      return Cred.encode(this).finish();
    },
  } as any,
  responseType: {
    deserializeBinary(data: Uint8Array) {
