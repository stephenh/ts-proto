...
export function dividerData_DividerTypeFromJSON(
  object: any,
...
export function dividerData_DividerTypeToJSON(
  object: DividerData_DividerType,
): string {
  switch (object) {
...
export function dividerData_DividerTypeToNumber(
  object: DividerData_DividerType,
): number {
  switch (object) {
    case DividerData_DividerType.DOUBLE:
...
export const DividerData = {
  encode(
    ...
    Object.entries(message.typeMap).forEach(([key, value]) => {
      DividerData_TypeMapEntry.encode(
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          break;
        case 2:
          const entry2 = DividerData_TypeMapEntry.decode(
            reader,
            reader.uint32(),
          );
  ...
  fromJSON(object: any): DividerData {
    return {
      ...
      typeMap: isObject(object.typeMap)
        ? Object.entries(object.typeMap).reduce<
          { [key: string]: DividerData_DividerType }
        >((acc, [key, value]) => {
  ...
  fromPartial<I extends Exact<DeepPartial<DividerData>, I>>(
    object: I,
  ): DividerData {
    const message = createBaseDividerData();
...
export const DividerData_TypeMapEntry = {
  encode(
    message: DividerData_TypeMapEntry,
    writer: _m0.Writer = _m0.Writer.create(),
  ): _m0.Writer {
    if (message.key !== "") {
  ...
  fromJSON(object: any): DividerData_TypeMapEntry {
    return {
      ...
      value: isSet(object.value)
        ? dividerData_DividerTypeFromJSON(object.value)
        : DividerData_DividerType.DOUBLE,
    };
  },
  ...
  fromPartial<I extends Exact<DeepPartial<DividerData_TypeMapEntry>, I>>(
    object: I,
  ): DividerData_TypeMapEntry {
    const message = createBaseDividerData_TypeMapEntry();
