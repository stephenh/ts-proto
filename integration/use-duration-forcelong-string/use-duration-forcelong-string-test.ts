...
import { DurationHolder } from "./use-duration-forcelong-string";

describe("useDuration=string with forceLong=string", () => {
  describe("binary encoding", () => {
    it("round-trips a Duration as a field of an enclosing message", () => {
      const original: DurationHolder = {
        duration: { seconds: "9223372036854775807", nanos: 999_999_999 },
      };
  ...
  describe("JSON encoding", () => {
    describe("round-trips Duration values exactly", () => {
      describe("multi-second values", () => {
        it.each<[string, Duration]>([
          ...
          ["1_000_000_000 seconds + 1 nano", { seconds: "1000000000", nanos: 1 }],
          ["i64 max + 999_999_999 nanos", { seconds: "9223372036854775807", nanos: 999_999_999 }],
          ["i64 min, 0 nanos", { seconds: "-9223372036854775808", nanos: 0 }],
    ...
    describe("fromJSON returns the seconds field with the declared TypeScript type", () => {
      it("returns seconds as a string", () => {
        const d = Duration.fromJSON("1s");
        expect(typeof d.seconds).toEqual("string");
