...
import { DurationHolder } from "./use-duration";

describe("useDuration=string with forceLong=number", () => {
  describe("binary encoding", () => {
    it("round-trips a Duration as a field of an enclosing message", () => {
      const original: DurationHolder = {
        duration: { seconds: 9_007_199_254_740_991, nanos: 999_999_999 },
      };
  ...
  describe("JSON encoding", () => {
    describe("round-trips Duration values exactly", () => {
      describe("multi-second values", () => {
        it.each<[string, Duration]>([
          ["2^23 seconds + 999_999_999 nanos", { seconds: 8388608, nanos: 999_999_999 }],
          ["1_000_000_000 seconds + 1 nano", { seconds: 1_000_000_000, nanos: 1 }],
          ["MAX_SAFE_INTEGER + 999_999_999 nanos", { seconds: 9_007_199_254_740_991, nanos: 999_999_999 }],
          ["-MAX_SAFE_INTEGER, 0 nanos", { seconds: -9_007_199_254_740_991, nanos: 0 }],
      ...
      describe("sub-second values", () => {
        it.each<[string, Duration]>([
          ["1 nanosecond", { seconds: 0, nanos: 1 }],
          ["999 nanoseconds", { seconds: 0, nanos: 999 }],
        ])("round-trips %s exactly", (_label, duration) => {
          expect(Duration.fromJSON(Duration.toJSON(duration))).toEqual(duration);
        });
      });
    ...
    describe("fromJSON returns the seconds field with the declared TypeScript type", () => {
      it("returns seconds as a number", () => {
        const d = Duration.fromJSON("1s");
        expect(typeof d.seconds).toEqual("number");
        expect(d.seconds).toEqual(1);
      });
    });

    describe("toJSON emits canonical 0/3/6/9 fractional digits", () => {
      it("emits 0 fractional digits when nanos is zero", () => {
        expect(Duration.toJSON({ seconds: 1, nanos: 0 })).toEqual("1s");
        expect(Duration.toJSON({ seconds: 0, nanos: 0 })).toEqual("0s");
      });

      it("emits 3 fractional digits for millisecond precision", () => {
        expect(Duration.toJSON({ seconds: 1, nanos: 500_000_000 })).toEqual("1.500s");
      });

      it("emits 6 fractional digits for microsecond precision", () => {
        expect(Duration.toJSON({ seconds: 1, nanos: 1_000 })).toEqual("1.000001s");
      });

      it("emits 9 fractional digits for nanosecond precision", () => {
        expect(Duration.toJSON({ seconds: 1, nanos: 1 })).toEqual("1.000000001s");
        expect(Duration.toJSON({ seconds: 0, nanos: 1 })).toEqual("0.000000001s");
      });
    });
    ...
    describe("fromJSON accepts non-canonical fractional digit counts", () => {
      it("parses 1, 2, 4 fractional digit forms", () => {
        expect(Duration.fromJSON("1.5s")).toEqual({ seconds: 1, nanos: 500_000_000 });
        expect(Duration.fromJSON("1.50s")).toEqual({ seconds: 1, nanos: 500_000_000 });
        expect(Duration.fromJSON("1.5000s")).toEqual({ seconds: 1, nanos: 500_000_000 });
      });

      it("parses negative durations", () => {
        expect(Duration.fromJSON("-1.500s")).toEqual({ seconds: -1, nanos: -500_000_000 });
        expect(Duration.fromJSON("-0.000000001s")).toEqual({ seconds: 0, nanos: -1 });
      });
    });
