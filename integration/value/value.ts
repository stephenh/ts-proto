...
export const ValueMessage = {
  encode(
    ...
    if (message.value !== undefined) {
      Value.encode(Value.wrap(message.value), writer.uint32(10).fork())
        .ldelim();
    }
    if (message.anyList !== undefined) {
      ListValue.encode(
        ListValue.wrap(message.anyList),
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          break;
        case 2:
          message.anyList = ListValue.unwrap(
            ListValue.decode(reader, reader.uint32()),
          );
          break;
        case 3:
          message.repeatedAny.push(
            Value.unwrap(Value.decode(reader, reader.uint32())),
          );
          break;
        case 4:
          message.repeatedStrings.push(
  ...
  fromJSON(object: any): ValueMessage {
    return {
      ...
      repeatedAny: Array.isArray(object?.repeatedAny)
        ? [...object.repeatedAny]
        : [],
      repeatedStrings: Array.isArray(object?.repeatedStrings)
